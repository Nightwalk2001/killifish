import {updateTank, updateTanks, useModal, useName, useSearch} from "@/hooks"
...
export const Tanks = () => {
  ...
  return <main className={"pl-8 pr-6 my-4"}>
    <div className={"flex space-x-4 my-4"}>
      ...
      <button onClick={handleSelectAll} className={count === selected.length ? "text-violet-400" : "text-gray-300"}>
        select all
      </button>
    </div>
    ...
    <Table>
      <div className={"table-header-group text-center"}>
        <div className={"table-row"}>
          ...
          <div role={"button"} className={thStyle}>
          ...
          <div className={thStyle}>amount</div>
          <div className={thStyle}>genotype</div>
          <div className={thStyle}>sexual</div>
          <div className={thStyle}>birthday</div>
          <div className={thStyle}>operations</div>
        </div>
      </div>
      <Tbody>
        {tanks && tanks.map((d: Tank, i: number) =>
          <div key={d.id} className={"table-row text-center text-gray-700 odd:bg-gray-50/50"}>
            <div className={clsx(tdStyle, "py-2")}>
              <input
                type="checkbox"
                checked={selected.includes(d.id)}
                className={"w-5 h-5 rounded-sm border-1.5"}
                onChange={() => handleSelect(d.id)}
              />
              <span className={"ml-2"}>{d.id}</span>
            ...
            <div className={clsx(tdStyle, "w-full truncate")}>{d.genotype}</div>
            <div className={tdStyle}>{d.sexual ?? <div className={"text-gray-400/50 italic"}>&lt;null&gt;</div>}</div>
